    ...
    I2C: Write<Error = E> + WriteRead<Error = E>,
{
    ...
    fn write_register(&mut self, register: Register, value: u8) -> Result<(), E> {
        self.i2c.write(self.address, &[register.into(), value])
    }
    ...
    pub fn reset(&mut self) -> Result<(), E> {
        // Soft reset device
        self.write_register(Register::CNTL3.into(), 0x01)?;
        Ok(())
    }

    pub fn set_mode(&mut self, mode: Mode) -> Result<(), E> {
        self.write_register(Register::CNTL2.into(), Mode::PowerDown.into())?;
        std::thread::sleep(std::time::Duration::from_micros(100));
        self.write_register(Register::CNTL2.into(), mode.into())?;
        Ok(())
    }
...
mod tests {
    ...
    fn set_mode_and_read_sensor() {
        ...
        let (x, y, z) = sensor.read_mag().expect("Error reading magnetometer");
